...
class GmediaProcessor_Library extends GmediaProcessor {
	...
	public function query_args() {
		...
		$args['album__in']        = array_filter( array_map( 'intval', (array) $gmCore->_get( 'album__in' ) ) );
		$args['album__not_in']    = array_filter( array_map( 'intval', (array) $gmCore->_get( 'album__not_in' ) ) );
		$args['tag_id']           = (int) $gmCore->_get( 'tag_id' );
		$args['tag__in']          = array_filter( array_map( 'intval', (array) $gmCore->_get( 'tag__in' ) ) );
		$args['tag__and']         = array_filter( array_map( 'intval', (array) $gmCore->_get( 'tag__and' ) ) );
		$args['tag__not_in']      = array_filter( array_map( 'intval', (array) $gmCore->_get( 'tag__not_in' ) ) );
		$args['cat']              = (int) $gmCore->_get( 'cat' );
		$args['category__in']     = array_filter( array_map( 'intval', (array) $gmCore->_get( 'category__in' ) ) );
		$args['category__not_in'] = array_filter( array_map( 'intval', (array) $gmCore->_get( 'category__not_in' ) ) );
		$args['category__and']    = array_filter( array_map( 'intval', (array) $gmCore->_get( 'category__and' ) ) );
