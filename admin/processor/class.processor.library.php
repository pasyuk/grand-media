...
class GmediaProcessor_Library extends GmediaProcessor {
	...
	public function __construct() {
		...
		$this->url            = add_query_arg( [ 'mode' => $this->mode, 'stack' => $stack, 'filter' => $filter, 'edit_term' => $this->edit_term ], $this->url );
	...
	public function admin_enqueue_scripts() {
		if ( 'edit' === $this->mode ) {
			if ( 'false' === $this->user_options['library_edit_quicktags'] ) {
				...
				add_action( 'admin_footer', [ $this, 'wpLink' ] );
			}
		}
	}
	...
	protected function processor() {
		...
		if ( in_array( $this->mode, [ 'select_single', 'select_multiple' ], true ) ) {
			$this->display_mode = 'grid';
		}
		if ( isset( $_GET['display_mode'] ) ) {
			$display_mode = $_GET['display_mode'];
			if ( in_array( $display_mode, [ 'grid', 'list' ], true ) ) {
				$view               = $this->gmediablank ? '_frame' : '';
				$this->user_options = array_merge( $this->user_options, [ "display_mode_gmedia{$view}" => $display_mode ] );
		...
		if ( isset( $_POST['quick_gallery'] ) ) {
			...
			do {
				...
				$gallery['query'] = array_merge( $gallery['query'], [ 'order' => 'ASC', 'orderby' => 'gmedia__in' ] );
				...
				$this->msg[] = sprintf( __( 'Gallery "%s" successfully saved. Shortcode: [gmedia id=%d]', 'grand-media' ), esc_html( $gallery['name'] ), $term_id );
			} while ( 0 );
		}

		if ( isset( $_POST['filter_categories'] ) ) {
			if ( false !== ( $term = $gmCore->_post( 'cat' ) ) ) {
				$location = add_query_arg( [ 'category__in' => implode( ',', $term ) ], $this->url );
				wp_redirect( $location );
				exit;
			}
		}
		if ( isset( $_POST['filter_albums'] ) ) {
			if ( false !== ( $term = $gmCore->_post( 'alb' ) ) ) {
				$location = add_query_arg( [ 'album__in' => implode( ',', $term ) ], $this->url );
				wp_redirect( $location );
				exit;
			}
		}
		if ( isset( $_POST['filter_tags'] ) ) {
			if ( false !== ( $term = $gmCore->_post( 'tag_ids' ) ) ) {
				$location = add_query_arg( [ 'tag__in' => $term ], $this->url );
				wp_redirect( $location );
				exit;
			}
		}
		if ( isset( $_POST['filter_author'] ) ) {
			...
			$location = add_query_arg( [ 'author' => (int) $authors ], $this->url );
			wp_redirect( $location );
			exit;
		}
			...
			if ( isset( $_POST['assign_album'] ) ) {
				...
				if ( $gmCore->caps['gmedia_terms'] ) {
					...
					if ( ! $gmCore->caps['gmedia_edit_others_media'] ) {
						$selected_items = $gmDB->get_gmedias( [ 'fields' => 'ids', 'author' => $user_ID, 'gmedia__in' => $ids ] );
						...
						foreach ( $selected_items as $item ) {
							...
							if ( 'custom' === $b_filename && ( $gmCore->caps['gmedia_delete_others_media'] || ( $item_author === $user_ID ) ) ) {
								...
								if ( ! empty( $filename_custom ) && ( '{filename}' !== $filename_custom ) ) {
									...
									$filename_vars = [ '{filename}' => $gmuid['filename'], '{id}' => $gmedia['ID'] ];
			...
			if ( $do_gmedia ) {
				...
				if ( 'recreate' === $do_gmedia ) {
					...
					if ( $gmCore->caps['gmedia_edit_media'] ) {
						...
						if ( $count ) {
							if ( ! $gmCore->caps['gmedia_edit_others_media'] ) {
								$edit_items     = $gmDB->get_gmedias( [ 'fields' => 'ids', 'author' => $user_ID, 'mime_type' => 'image', 'gmedia__in' => $selected_items ] );
								$selected_items = $edit_items;
							} else {
								$selected_items = $gmDB->get_gmedias( [ 'fields' => 'ids', 'mime_type' => 'image', 'gmedia__in' => $selected_items ] );
							}
							if ( ( $count = count( $selected_items ) ) ) {
			...
			if ( $gmCore->caps['gmedia_delete_media'] ) {
				...
				if ( ! empty( $selected_items ) ) {
					if ( ! $gmCore->caps['gmedia_delete_others_media'] ) {
						$delete_items = $gmDB->get_gmedias( [ 'fields' => 'ids', 'author' => $user_ID, 'gmedia__in' => $selected_items ] );
		...
		if ( $do_gmedia ) {
			...
			$remove_args = array_merge( [ 'do_gmedia', 'ids' ], $_wpnonce );
	...
	public function query_args() {
		...
		if ( ! empty( $query_args['album__in'] ) ) {
			if ( isset( $query_args['within_album'] ) ) {
				...
				if ( ! empty( $query_args['with_album__not_in'] ) ) {
					$albums_names = $gmDB->get_terms( 'gmedia_album', [ 'fields' => 'names', 'global' => $args['author__in'], 'include' => $query_args['with_album__not_in'] ] );
				}
				array_push( $albums_names, __( 'Hide items without album', 'grand-media' ) );
			} else {
				$filter_title = __( 'Filter Album', 'grand-media' );
				$albums_names = $gmDB->get_terms( 'gmedia_album', [ 'fields' => 'names', 'global' => $args['author__in'], 'include' => $query_args['album__in'] ] );
			}
		...
		if ( ! empty( $query_args['album__not_in'] ) ) {
			if ( isset( $query_args['without_album'] ) ) {
				...
				if ( ! empty( $query_args['with_album__in'] ) ) {
					$albums_names = $gmDB->get_terms( 'gmedia_album', [ 'fields' => 'names', 'global' => $args['author__in'], 'include' => $query_args['with_album__in'] ] );
				}
				array_push( $albums_names, __( 'Show items without album', 'grand-media' ) );
			} else {
				$filter_title = __( 'Exclude Album', 'grand-media' );
				$albums_names = $gmDB->get_terms( 'gmedia_album', [ 'fields' => 'names', 'global' => $args['author__in'], 'include' => $query_args['album__not_in'] ] );
			}
			if ( ! empty( $albums_names ) ) {
		...
		if ( ! empty( $query_args['category__in'] ) ) {
			if ( isset( $query_args['within_category'] ) ) {
				...
				if ( ! empty( $query_args['with_category__not_in'] ) ) {
					$category_names = $gmDB->get_terms( 'gmedia_category', [ 'fields' => 'names', 'include' => $query_args['with_category__not_in'] ] );
				}
				...
				$category_names = $gmDB->get_terms( 'gmedia_category', [ 'fields' => 'names', 'include' => $query_args['category__in'] ] );
			}
		...
		if ( ! empty( $query_args['category__not_in'] ) ) {
			if ( isset( $query_args['without_category'] ) ) {
				...
				if ( ! empty( $query_args['with_category__in'] ) ) {
					$category_names = $gmDB->get_terms( 'gmedia_category', [ 'fields' => 'names', 'include' => $query_args['with_category__in'] ] );
				}
				$category_names = array_push( $category_names, __( 'Show items without categories', 'grand-media' ) );
			} else {
				$filter_title   = __( 'Exclude Category', 'grand-media' );
				$category_names = $gmDB->get_terms( 'gmedia_category', [ 'fields' => 'names', 'include' => $query_args['category__not_in'] ] );
			}
			if ( ! empty( $category_names ) ) {
		...
		if ( ! empty( $query_args['tag__in'] ) ) {
			$tag_names = $gmDB->get_terms( 'gmedia_tag', [ 'fields' => 'names', 'include' => $query_args['tag__in'] ] );
			if ( ! empty( $tag_names ) ) {
		...
		if ( ! empty( $query_args['tag__not_in'] ) ) {
			$tag_names = $gmDB->get_terms( 'gmedia_tag', [ 'fields' => 'names', 'include' => $query_args['tag__not_in'] ] );
			if ( ! empty( $tag_names ) ) {
		...
		if ( ! empty( $args['terms_relation'] ) ) {
			$this->filters['terms_relation'] = [
				...
				'filter' => [ strip_tags( $args['terms_relation'] ) ],
			];
		}
