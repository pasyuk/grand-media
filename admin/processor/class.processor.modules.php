		...
		if ( isset( $_POST['module_preset_save'] ) || isset( $_POST['module_preset_save_default'] ) || isset( $_POST['module_preset_save_global'] ) ) {
			...
			do {
				...
				if ( $edit_preset ) {
					...
					$location = add_query_arg( [ 'preset' => $term_id, 'message' => 'save' ], $this->url );
			...
			if ( ! empty( $_FILES['modulezip']['tmp_name'] ) ) {
				...
				if ( false === @move_uploaded_file( $_FILES['modulezip']['tmp_name'], $to_folder . $filename ) ) {
					...
					if ( is_wp_error( $result ) ) {
						...
						$this->msg[] = sprintf( __( "The `%s` file unzipped to module's directory", 'grand-media' ), esc_attr( $filename ) );
					}
				}
				gmedia_delete_transients( 'gm_cache' );
			} else {
		...
		if ( isset( $_GET['delete_module'] ) ) {
			if ( $gmCore->_get( '_wpnonce_module_delete' ) ) {
				...
				if ( $mfold && file_exists( $mpath ) ) {
					...
					$location = remove_query_arg( [ '_wpnonce_module_delete' ] );
					set_transient( 'gmedia_module_deleted', sprintf( __( "The `%s` module folder was deleted", 'grand-media' ), esc_attr( $mpath ) ), 60 );
					wp_redirect( $location );
				}
