...
?>
<form method="post" id="gmedia-edit-term" name="gmEditTerm" class="panel-body" data-id="<?php echo absint( $term->term_id ); ?>" action="<?php echo esc_url( gm_get_admin_url() ); ?>">
	...
	<div class="row">
		<div class="col-xs-6">
			...
			<div class="form-group">
				...
				<?php
				wp_editor( esc_textarea( $term->description ), "album{$term->term_id}_description", [
		...
		<div class="col-xs-6">
			<div class="row">
				<div class="col-xs-6">
					...
					<p><a href="<?php echo esc_url( $term->cloud_link ); ?>" target="_blank"><?php _e( 'View GmediaCloud Page', 'grand-media' ); ?></a></p>
					<?php if ( $term->post_link ) { ?>
						<p><a href="<?php echo esc_url( $term->post_link ); ?>" target="_blank"><?php _e( 'View Wordpress Post', 'grand-media' ); ?></a></p>
					<?php }

					/*if(isset($term->comment_status)){ ?>
					<div class="form-group">
						<a href="<?php echo esc_url( add_query_arg(array('page' => 'GrandMedia', 'gmediablank' => 'comments', 'gmedia_term_id' => $term->term_id), $gmProcessor->url) ); ?>" data-target="#previewModal" data-width="900" data-height="500" class="preview-modal gmpost-com-count pull-right" title="<?php echo esc_attr('Comments', 'grand-media'); ?>">
				...
				<div class="col-xs-6">
					...
					<?php if ( isset( $term->post_date ) ) { ?>
						<div class="form-group">
							...
							<div class="input-group gmedia_date input-group-sm" data-date-format="YYYY-MM-DD HH:mm:ss">
								<input name="term[post_date]" type="text" readonly="readonly" class="form-control input-sm" value="<?php echo esc_attr( $term->post_date ); ?>" tabindex="-1"/>
					...
					<div class="form-group">
						...
						<div class="input-group">
							<input type="text" class="form-control input-sm gmedia-cover-id" name="term[meta][_cover]" value="<?php echo esc_attr( $cover_id ); ?>" placeholder="<?php _e( 'Gmedia Image ID', 'grand-media' ); ?>"/>
					...
					<div class="gm-img-thumbnail gmedia-cover-image"><?php
						if ( ( $cover_id = intval( $cover_id ) ) ) {
							if ( ( $cover = $gmDB->get_gmedia( $cover_id ) ) ) {
								?><img src="<?php echo esc_url( $gmCore->gm_get_media_image( $cover, 'thumb', true ) ); ?>" alt="" /><?php
							} else {
