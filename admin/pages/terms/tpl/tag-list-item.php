		...
		<div class="col-xs-6 term-label">
			<div class="checkbox">
				...
				<?php if ( $item->allow_edit ) { ?>
					...
					<span class="edit_tag_form" style="display:none;"><input class="edit_tag_input" type="text" data-tag_id="<?php echo absint( $item->term_id ); ?>" name="gmedia_tag_name[<?php echo absint( $item->term_id ); ?>]" value="<?php echo esc_attr( $item->name ); ?>" placeholder="<?php echo esc_attr( $item->name ); ?>"/><a href="#tag_<?php echo absint( $item->term_id ); ?>" class="edit_tag_save btn btn-link glyphicon glyphicon-pencil"></a></span>
				<?php } else { ?>
