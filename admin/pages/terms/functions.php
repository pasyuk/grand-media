...
function gmedia_term_item_actions( $item ) {
	...
	$filter_href  = $gmCore->get_admin_url( [ 'page' => 'GrandMedia', "{$taxterm}__in" => $item->term_id ], [], true );
	$filter_class = 'gm_filter_in_lib';
	$count        = '';
	if ( in_array( $item->taxonomy, [ 'gmedia_album', 'gmedia_tag', 'gmedia_category' ], true ) ) {
...
function gmedia_term_album_after_panel( $term ) {
	...
	if ( ! empty( $gmProcessorLibrary->query_args[ $taxin ] ) ) {
		$gmProcessorLibrary->query_args["{$gmProcessor->taxterm}__and"] = wp_parse_id_list( array_merge( $gmProcessorLibrary->query_args[ $taxin ], [ $term->term_id ] ) );
		unset( $gmProcessorLibrary->query_args[ $taxin ] );
	} else {
		$gmProcessorLibrary->query_args[ $taxin ] = [ (int) $term->term_id ];
	}
