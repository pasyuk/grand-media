...
function grandWPMedia() {
	...
	$url = add_query_arg( [ 'page' => $gmProcessor->page ], admin_url( 'admin.php' ) );
	...
	$gm_qty = [ 'total' => '', 'image' => '', 'audio' => '', 'video' => '', 'text' => '', 'application' => '', 'other' => '' ];
			...
			<div class="btn-toolbar pull-left">
				...
				<div class="btn-group">
					...
					<?php if ( ! empty( $gmDB->filter ) ) { ?>
						<a class="btn btn-warning" title="<?php _e( 'Reset Filter', 'grand-media' ); ?>" rel="total" href="<?php echo esc_url( $url ); ?>"><?php _e( 'Filter', 'grand-media' ); ?></a>
					<?php } else { ?>
					...
					<ul class="dropdown-menu" role="menu">
						...
						<li class="total<?php echo in_array( 'total', $curr_mime, true ) ? ' active' : ''; ?>"><a rel="total" href="<?php echo esc_url( $gmCore->get_admin_url( [], [ 'mime_type', 'pager' ] ) ); ?>"><?php echo $gm_qty['total'] . __( 'All', 'grand-media' ); ?></a></li>
						<li class="image<?php echo ( in_array( 'image', $curr_mime, true ) ? ' active' : '' ) . ( $gmDbCount['image'] ? '' : ' disabled' ); ?>">
							<a rel="image" href="<?php echo esc_url( $gmCore->get_admin_url( [ 'mime_type' => 'image' ], [ 'pager' ] ) ); ?>"><?php echo $gm_qty['image'] . __( 'Images', 'grand-media' ); ?></a></li>
						<li class="audio<?php echo ( in_array( 'audio', $curr_mime, true ) ? ' active' : '' ) . ( $gmDbCount['audio'] ? '' : ' disabled' ); ?>">
							<a rel="audio" href="<?php echo esc_url( $gmCore->get_admin_url( [ 'mime_type' => 'audio' ], [ 'pager' ] ) ); ?>"><?php echo $gm_qty['audio'] . __( 'Audio', 'grand-media' ); ?></a></li>
						<li class="video<?php echo ( in_array( 'video', $curr_mime, true ) ? ' active' : '' ) . ( $gmDbCount['video'] ? '' : ' disabled' ); ?>">
							<a rel="video" href="<?php echo esc_url( $gmCore->get_admin_url( [ 'mime_type' => 'video' ], [ 'pager' ] ) ); ?>"><?php echo $gm_qty['video'] . __( 'Video', 'grand-media' ); ?></a></li>
						<li class="application<?php echo ( ( in_array( 'application', $curr_mime, true ) || in_array( 'text', $curr_mime, true ) ) ? ' active' : '' ) . ( $gmDbCount['other'] ? '' : ' disabled' ); ?>">
							<a rel="application" href="<?php echo esc_url( $gmCore->get_admin_url( [ 'mime_type' => 'application,text' ], [ 'pager' ] ) ); ?>"><?php echo $gm_qty['other'] . __( 'Other', 'grand-media' ); ?></a></li>
				...
				<form class="btn-group" id="gm-selected-btn" name="gm-selected-form" action="<?php echo esc_url( add_query_arg( [ 'filter' => 'selected' ], $url ) ); ?>" method="post">
			...
			<table class="table table-striped table-hover table-condenced" cellspacing="0">
				...
				<tr>
					...
					<th class="id" title="<?php _e( 'Sort by ID', 'grand-media' ); ?>">
						...
						<a href="<?php echo esc_url( $gmCore->get_admin_url( [ 'orderby' => 'ID', 'order' => $new_order ] ) ); ?>"><?php _e( 'ID', 'grand-media' ); ?></a>
					</th>
					...
					<th class="title" title="<?php _e( 'Sort by Title', 'grand-media' ); ?>">
						...
						<a href="<?php echo esc_url( $gmCore->get_admin_url( [ 'orderby' => 'title', 'order' => $new_order ] ) ); ?>"><?php _e( 'Title', 'grand-media' ); ?></a>
					</th>
				...
				<?php foreach ( $wpMediaLib as $item ) {
					$is_selected = in_array( $item->ID, $gmProcessor->selected_items, true ) ? true : false;
					$image       = wp_get_attachment_image( $item->ID, [ 50, 50 ], false );
					if ( ! $image ) {
					...
					<tr data-id="<?php echo absint( $item->ID ); ?>">
						...
						<td class="file">
							<span><a href="<?php echo esc_url( admin_url( 'media.php?action=edit&amp;attachment_id=' . $item->ID ) ); ?>"><?php echo $image; ?></a></span>
						</td>
