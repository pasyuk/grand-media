...
class GrandMedia_Gallery_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct( 'gmedia_gallery_widget', // Base ID.
			...
			[ 'description' => esc_html__( 'Display Gmedia Gallery in the widget', 'grand-media' ), ] // Args.
		);
	}
	...
	public function form( $instance ) {
		...
		$gmedia_terms = $gmDB->get_terms( 'gmedia_gallery', [ 'status' => [ 'publish', 'private' ], 'orderby' => 'name', 'order' => 'ASC' ] );
		?>
		<p>
		...
		<p>
			...
			<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'term_id' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'term_id' ) ); ?>">
				...
				foreach ( $gmedia_terms as &$item ) {
					...
					echo "<option value='{$item->term_id}' {$selected}>" . esc_html( $item->name . " [{$item->status}] " ) . ( $item->author_name ? sprintf( __( 'by %s', 'grand-media' ), esc_html( $item->author_name ) ) : '(' . __( 'deleted author', 'grand-media' ) . ')' ) . '</option>';
				}
				?>
			</select>
		</p>
	...
	public function widget( $args, $instance ) {
		if ( empty( $instance['term_id'] ) ) {
			global $gmDB;
			$term_id = $gmDB->get_terms( 'gmedia_gallery', [ 'status' => [ 'publish' ], 'fields' => 'ids', 'number' => 1, 'orderby' => 'rand' ] );
			if ( empty( $term_id ) ) {
				return;
			}
...
class GrandMedia_Album_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct( 'gmedia_album_widget', // Base ID
			...
			[ 'description' => esc_html__( 'Display Gmedia Album in the widget', 'grand-media' ), ] // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		if ( empty( $instance['term_id'] ) ) {
			global $gmDB;
			$term_id = $gmDB->get_terms( 'gmedia_album', [ 'status' => [ 'publish' ], 'fields' => 'ids', 'number' => 1, 'orderby' => 'rand', 'hide_empty' => true ] );
			if ( empty( $term_id ) ) {
				return;
			}
			$instance['term_id'] = $term_id[0];
	...
	public function form( $instance ) {
		global $gmDB;
		$title        = ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( 'New title', 'grand-media' );
		$gmedia_terms = $gmDB->get_terms( 'gmedia_album', [ 'status' => [ 'publish', 'private' ], 'orderby' => 'name', 'order' => 'ASC' ] );
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_attr_e( 'Title:', 'grand-media' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>">
		...
		<p>
			...
			<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'term_id' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'term_id' ) ); ?>">
				...
				foreach ( $gmedia_terms as &$item ) {
					...
					$selected = $instance['term_id'] ? selected( $instance['term_id'], $item->term_id, false ) : '';
					echo "<option value='{$item->term_id}' {$selected}>" . esc_html( $item->name . " ({$item->count}) [{$item->status}] " ) . ( $item->author_name ? sprintf( __( 'by %s', 'grand-media' ), esc_html( $item->author_name ) ) : '(' . __( 'deleted author', 'grand-media' ) . ')' ) . '</option>';
				}
				?>
			</select>
		</p>
