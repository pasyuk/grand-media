...
function gmedia_add_media_popup_enqueue_scripts() {
	...
	if ( did_action( 'media_upload_gmedia_library' ) && ( 'upload' === $action ) && current_user_can( 'gmedia_upload' ) ) {
		if ( current_user_can( 'gmedia_terms' ) ) {
			wp_enqueue_style( 'selectize', $gmCore->gmedia_url . '/assets/selectize/selectize.bootstrap3.css', [ 'gmedia-bootstrap' ], '0.8.5', 'screen' );
			wp_enqueue_script( 'selectize', $gmCore->gmedia_url . '/assets/selectize/selectize.min.js', [ 'jquery' ], '0.8.5' );
		}
		...
		wp_enqueue_script( 'jquery.ui.plupload', $gmCore->gmedia_url . '/assets/plupload/jquery.ui.plupload/jquery.ui.plupload.min.js', [ 'gmedia-plupload', 'jquery-ui-full' ], '2.1.2' );
	}
	...
	<div class="panel panel-default" id="gmedia-container">
		<div class="panel-heading clearfix">
			...
			<div class="btn-group" style="margin-right:20px;">
				<a class="btn btn-primary" target="_blank" href="<?php echo esc_url( add_query_arg( [ 'page' => 'GrandMedia_Modules' ], admin_url( 'admin.php' ) ) ); ?>"><?php _e( 'Create New Gallery', 'grand-media' ); ?></a>
			</div>
				...
				<div class="col-xs-7 col-md-9" style="padding: 0">
					<div class="list-group">
						...
						if ( count( $gmediaTerms ) ) {
							$lib_url = add_query_arg( [ 'page' => 'GrandMedia' ], admin_url( 'admin.php' ) );
							foreach ( $gmediaTerms as $term ) {
								...
								?>
								<div class="gmedia-insert-item list-group-item clearfix d-row<?php echo $list_row_class; ?>" id="list-item-<?php echo absint( $term->term_id ); ?>" data-id="<?php echo absint( $term->term_id ); ?>" data-type="<?php echo esc_attr( $term_meta['_module'] ); ?>">
									...
									<p class="media-meta hidden" style="font-weight:bold">
										<span class="label label-default"><?php _e( 'Shortcode', 'grand-media' ); ?>:</span> [gmedia id=<?php echo absint( $term->term_id ); ?>]
									...
									<p class="media-meta clear hidden">
										<span class="clearfix">
											<span class="media-object pull-left" style="width:85px;margin-right:5px;">
												<?php if ( ! $broken ) { ?>
													<span class="thumbnail"><img src="<?php echo esc_url( $module['url'] . '/screenshot.png' ); ?>" alt="<?php echo esc_attr( $term->name ); ?>"/></span>
												<?php } else { ?>
									...
									<?php if ( current_user_can( 'gmedia_gallery_manage' ) ) {
										if ( ! ( ( (int) $term->global !== $user_ID ) && ! current_user_can( 'gmedia_edit_others_media' ) ) ) {
											?>
											<p class="media-meta hidden"><a target="_blank" href="<?php echo esc_url( add_query_arg( [
...
function gmedia_add_media_terms() {
	...
	$url = add_query_arg( [ 'post_id' => $post_id, 'tab' => 'gmedia_terms', 'chromeless' => true ], admin_url( 'media-upload.php' ) );

	$taxonomy = $gmCore->_get( 'term', 'gmedia_album' );
	if ( ! in_array( $taxonomy, [ 'gmedia_album', 'gmedia_tag', 'gmedia_category' ], true ) ) {
	...
	switch ( $taxonomy ) {
		case 'gmedia_album':
			...
			$args['global'] = $gmCore->_get( 'author', $gmCore->caps['gmedia_edit_others_media'] ? '' : [ 0, $user_ID ] );
			if ( ! $gmCore->caps['gmedia_show_others_media'] ) {
				$args['global'] = wp_parse_id_list( $args['global'] );
				$args['global'] = array_intersect( [ 0, $user_ID ], $args['global'] );
		...
		<div class="panel-heading clearfix">
			...
			<div class="btn-group" style="margin-right:20px;">
				<a class="btn btn<?php echo ( 'gmedia_album' === $taxonomy ) ? "-primary active" : '-default'; ?>"
					href="<?php echo esc_url( add_query_arg( [ 'term' => 'gmedia_album' ], $url ) ); ?>"><?php _e( 'Albums', 'grand-media' ); ?></a>
				<a class="btn btn<?php echo ( 'gmedia_category' === $taxonomy ) ? "-primary active" : '-default'; ?>"
					href="<?php echo esc_url( add_query_arg( [ 'term' => 'gmedia_category' ], $url ) ); ?>"><?php _e( 'Categories', 'grand-media' ); ?></a>
				<a class="btn btn<?php echo ( 'gmedia_tag' === $taxonomy ) ? "-primary active" : '-default'; ?>"
					href="<?php echo esc_url( add_query_arg( [ 'term' => 'gmedia_tag' ], $url ) ); ?>"><?php _e( 'Tags', 'grand-media' ); ?></a>
			</div>
							...
							foreach ( $gmediaTerms as $item ) {
								...
								if ( $item->count ) {
									if ( 'gmedia_album' === $taxonomy ) {
										...
										$term_meta = array_map( 'reset', $term_meta );
										$term_meta = array_merge( [ '_orderby' => $gmGallery->options['in_album_orderby'], '_order' => $gmGallery->options['in_album_order'] ], $term_meta );
										$args      = [
											'no_found_rows' => true,
											'per_page'      => $per_page,
										...
										$term_meta = $gmDB->get_metadata( 'gmedia_term', $item->term_id );
										$term_meta = array_map( 'reset', $term_meta );
										$term_meta = array_merge( [ '_orderby' => $gmGallery->options['in_category_orderby'], '_order' => $gmGallery->options['in_category_order'] ], $term_meta );
										$args      = [
											'no_found_rows' => true,
											'per_page'      => $per_page,
								...
								$module_preset = esc_html( $gmedia_modules['in'][ $mfold ]['title'] . $by_author . ' - ' . $preset_name );
								?>
								<div class="list-group-item term-list-item d-row<?php echo $list_row_class; ?>">
									...
									<div class="term-info hidden">
										...
										$term_meta = $gmDB->get_metadata( 'gmedia_term', $item->term_id );
										$term_meta = array_map( 'reset', $term_meta );
										$term_meta = array_merge( [ '_orderby' => 'ID', '_order' => 'DESC' ], $term_meta );
										$tax_name  = [
										...
										<p><strong><?php echo $tax_name[ $taxonomy ]; ?>:</strong> <?php echo esc_html( $item_name ); ?>
											<br/><strong><?php _e( 'ID', 'grand-media' ); ?>:</strong> <?php echo absint( $item->term_id ); ?>
				...
				<div class="col-xs-7 col-md-9" style="text-align:justify;white-space:normal;">
					...
					if ( count( $gmediaQuery ) ) {
						foreach ( $gmediaQuery as $item ) {
							...
							?>
							<form class="thumbnail" id="list-item-<?php echo absint( $item->ID ); ?>" data-id="<?php echo absint( $item->ID ); ?>" data-type="<?php echo esc_attr( $item->type ); ?>">
								...
								<div class="media-upload-form" style="display:none;">
									...
									<div class="form-group">
										...
										<input name="link" type="text" class="customurl form-control input-sm" value="<?php echo esc_attr( $item->link ); ?>" placeholder="http://"/>
						...
						<div class="list-group-item">
							<div class="well well-lg text-center">
								...
								<?php if ( $gmCore->caps['gmedia_upload'] ) { ?>
									<p>
										<a target="_blank" href="<?php echo esc_url( admin_url( 'admin.php?page=GrandMedia_AddMedia' ) ); ?>" class="btn btn-success"><span class="glyphicon glyphicon-plus"></span> <?php _e( 'Add Media', 'grand-media' ); ?>
										</a></p>
								<?php } ?>
							</div>
						</div>
