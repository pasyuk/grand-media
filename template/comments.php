...
if(have_comments()) : ?>
    <ol class="gmediacommentlist">
        <?php
        $wp_list_comments_args = array('per_page'    => 100,
                                       'avatar_size' => 25,
                                       'format'      => 'xhtml',
        );
...
<?php else : // this is displayed if there are no comments so far ?>
    ...
    <?php else : // comments are closed ?>
        ...
        <p class="gmedia_nocomments"><?php _e('Comments are closed.'); ?></p>
...
<?php if(comments_open()) :
    ...
    $fields            = array('author' => '<p class="gmedia_comment-form-author"><input id="author" class="gmedia_comments-input" name="author" type="text" placeholder="' . __('Name') . '" value="' . esc_attr($commenter['comment_author']) . '" size="30" required="required" /></p>',
    ...
    $comment_form_args = array('fields'               => $fields,
                               ...
                               'must_log_in'          => '<p class="gmedia_must-log-in">' . sprintf(__('<a href="%s">Log in</a> to post a comment.'), wp_login_url(apply_filters('the_permalink', get_permalink($post_id)))) . '</p>',
                               'logged_in_as'         => '<p class="gmedia_logged-in-as">' . sprintf(__('Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s">Log out?</a>'), get_edit_user_link(), $user_identity, wp_logout_url(apply_filters('the_permalink', get_permalink($post_id)))) . '</p>',
